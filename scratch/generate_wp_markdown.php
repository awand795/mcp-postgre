...
echo "# Buku Panduan Administrator — DarkoAI Admin Panel\n\n";
echo "Selamat datang di panduan lengkap operasional Admin Panel DarkoAI. Dokumentasi ini disusun untuk membantu Anda mengelola seluruh aspek sistem mulai dari autentikasi, manajemen database, hingga konfigurasi AI tingkat lanjut.\n\n";
...
foreach ($guideData as $menu) {
    ...
    foreach ($menu['sections'] as $sec) {
        ...
        // Special RLS info box
        if ($sec['id'] === 'user-rls') {
            echo "#### ℹ️ Referensi Cepat: Operator Kondisi Filter\n\n";
            echo "| Operator | Arti | Contoh Penggunaan |\n";
            echo "| :--- | :--- | :--- |\n";
            echo "| `=` | Sama persis dengan nilai | `kode_cabang = 1271` |\n";
            echo "| `!=` | Tidak sama dengan nilai | `status != inactive` |\n";
            echo "| `>` | Lebih besar dari nilai | `total_penjualan > 1000000` |\n";
            echo "| `<` | Lebih kecil dari nilai | `umur < 30` |\n";
            echo "| `LIKE` | Mengandung teks tertentu | `nama_produk LIKE Beras` |\n";
            echo "| `IN` | Nilainya ada dalam daftar | `kota IN Jakarta,Bandung,Surabaya` |\n\n";
        }

        foreach ($sec['steps'] as $step) {
            $desc = str_replace(['<strong>', '</strong>'], ['**', '**'], $step['desc']);
            $desc = str_replace(['<code>', '</code>'], ['`', '`'], $desc);
            $desc = str_replace(['<br>', '<br/>', '<br />'], "\n", $desc);
            $desc = str_replace(['<kbd>', '</kbd>'], ['`', '`'], $desc);
            $desc = str_replace(['<em>', '</em>'], ['*', '*'], $desc);
            $desc = strip_tags($desc);

            echo "#### Langkah " . $step['no'] . ": " . $step['text'] . "\n\n";
            echo $desc . "\n\n";
            echo "![" . $step['label'] . "](" . $baseUrl . $step['img'] . ")\n\n";
            echo "* Gambar: " . $step['label'] . " *\n\n";
            echo "---\n\n";
        }
    }
}

echo "## Panduan Selesai\n\n";
echo "Dokumentasi ini mencakup seluruh fitur Admin Panel DarkoAI. Gunakan sebagai Standar Operasional Prosedur (SOP) administrasi sistem.\n";
