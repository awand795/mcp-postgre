...
<body class="p-6">
    ...
    <div class="w-full max-w-md glass-panel p-10 rounded-3xl">
        ...
        <form action="{{ route('register.store') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                <label class="block text-xs font-semibold text-[#A1A09A] uppercase tracking-wider mb-2 ml-1">Nama
                    Lengkap</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-[#A1A09A] uppercase tracking-wider mb-2 ml-1">Alamat
                    Email</label>
            ...
            <div>
                <label
                    class="block text-xs font-semibold text-[#A1A09A] uppercase tracking-wider mb-2 ml-1">Password</label>
                <input type="password" name="password" required class="w-full input-glass rounded-xl px-4 py-3 text-sm"
                    placeholder="••••••••">
                @error('password') <p class="text-red-500 text-[11px] mt-1 ml-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-xs font-semibold text-[#A1A09A] uppercase tracking-wider mb-2 ml-1">Konfirmasi
                    Password</label>
            ...
            <button type="submit"
                class="w-full btn-primary text-white font-bold py-3.5 rounded-xl shadow-lg mt-4 text-sm tracking-wide">
                DAFTAR SEKARANG
            </button>
        </form>
        ...
        <div class="mt-8 text-center">
            <p class="text-[#A1A09A] text-sm">
                Sudah punya akun?
                <a href="{{ route('login') }}"
                    class="text-[#f53003] hover:text-[#ff4433] font-semibold transition-colors">Masuk di sini</a>
            </p>
        </div>
    </div>
