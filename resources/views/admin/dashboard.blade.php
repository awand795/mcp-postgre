...
<div class="stats-grid">
    ...
    <div class="stat-card glass-card">
        ...
        <div class="stat-body">
            <span class="stat-label">{{ __('Total Roles') }}</span>
    ...
    <div class="stat-card glass-card">
        ...
        <div class="stat-body">
            <span class="stat-label">{{ __('Total Databases') }}</span>
    ...
    <div class="stat-card glass-card">
        ...
        <div class="stat-body">
            <span class="stat-label">{{ __('Total Tables') }}</span>
...
<div class="glass-card welcome-card">
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:1rem;">
        ...
        <div>
            <h3 style="font-size:1.1rem;font-weight:700;color:var(--text-main);">{{ __('Selamat Datang, Admin! 👋') }}</h3>
            <p style="color:var(--text-muted);font-size:0.85rem;margin-top:2px;">{{ __('Kelola sistem Darko AI melalui panel administrasi') }}</p>
        </div>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;">
        <a href="{{ route('admin.users') }}" class="quick-link">
            <i class="fas fa-users"></i> <span>{{ __('Kelola Users') }}</span>
        </a>
        <a href="{{ route('admin.roles') }}" class="quick-link">
            <i class="fas fa-user-shield"></i> <span>{{ __('Kelola Roles') }}</span>
        </a>
        <a href="{{ route('admin.databases') }}" class="quick-link">
            <i class="fas fa-database"></i> <span>{{ __('Kelola Database') }}</span>
        </a>
        <a href="{{ route('admin.ai_management') }}" class="quick-link">
            <i class="fas fa-brain"></i> <span>{{ __('Kelola AI') }}</span>
        </a>
    </div>
</div>
