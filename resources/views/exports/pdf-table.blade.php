...
<body>
    <div class="report-header">
        ...
        <div class="report-meta">{{ $generatedAt }} &bull; {{ count($rows) }} {{ __('Baris') }} &bull; {{ $colCount }} {{ __('Kolom') }}</div>
    </div>

    @if($chartImage)
    <div class="chart-container">
        <img src="{{ $chartImage }}" alt="{{ __('Chart') }}">
    </div>
    @endif
    ...
    @else
    <div class="no-data">{{ __('Tidak ada data untuk ditampilkan') }}</div>
    @endif

    <div class="footer">
        {{ __('Dihasilkan oleh darkotech AI') }} &bull; {{ date('Y') }} &bull; {{ __('Total: :count Baris', ['count' => count($rows)]) }}
    </div>
</body>
</html>
