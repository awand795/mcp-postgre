...
<head>
    <meta charset="utf-8">
    <title>{{ __('OTP Reset Password') }}</title>
</head>
<body style="font-family: Arial, sans-serif; background-color: #f4f4f5; padding: 20px; color: #333;">
    <div style="max-width: 500px; margin: 0 auto; background-color: #ffffff; padding: 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
        ...
        <p>{{ __('Halo,') }}</p>
        <p>{{ __('Anda menerima email ini karena kami menerima permintaan reset password untuk akun Anda.') }}</p>
        <p>{{ __('Berikut adalah kode OTP untuk melanjutkan proses reset password Anda:') }}</p>
        ...
        <p>{{ __('Kode OTP ini hanya berlaku selama 15 menit.') }}</p>
        <p>{{ __('Jika Anda tidak meminta reset password, Anda dapat mengabaikan email ini.') }}</p>
        <br>
        <p>{{ __('Salam,') }}</p>
        <p><strong>{{ __('Tim darkotech AI') }}</strong></p>
    </div>
</body>
</html>
